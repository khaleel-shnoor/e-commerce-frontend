...
export function Sidebar({ links, title, basePath = '', disabled = false }) {
  ...
  return (
    <aside className="w-64 shrink-0 border-r border-border bg-card min-h-[calc(100vh-4rem)] p-4 sticky top-16 flex flex-col">
      ...
      <nav className="space-y-1 flex-1">
        {links.map((link) => {
          const isProfile = link.to === '/profile' || link.to === 'profile';
          const linkDisabled = disabled && !isProfile;
          return (
            <NavLink
              key={link.to}
              to={linkDisabled ? '#' : `${basePath}${link.to}`}
              end={link.end}
...
export function MobileSidebarNav({ links, basePath = '', disabled = false }) {
  return (
